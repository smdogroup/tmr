...
class pyApproximationSpace(BaseUI):
    ...
    def createProblem(self, prob_name, **kwargs):
        ...
        self.problem = self.probCallBack(name=prob_name, comm=self.comm, geomLoader=self.geomLoader, assembler=self.assembler, **kwargs)

        # check the output of interest is present in the problem
        output_names = self.problem.getFunctionKeys()
        assert self.output_name in output_names, \
    ...
    def setAuxElements(self, geom_labels, aux_type, **kwargs):
        ...
        assert aux_type in valid_aux_types, \
            f"aux_type must be one of {valid_aux_types}"
        ...
        for name in geom_labels:
            ...
            for eid in elem_ids:
                ...
                if aux_type == "traction":
                    ...
                    aux = elem.createElementPressure(kwargs.get('faceIndex'), kwargs.get('p'))
                elif aux_type == "inertial":
                    aux = elem.createElementInertialForce(kwargs.get('inertiaVec'))
                elif aux_type == "centrifugal":
                    aux = elem.createElementCentrifugalForce(kwargs.get('omegaVec'), kwargs.get('rotCenter'), kwargs.get('firstOrder',False))
    ...
    def applyRefinement(self, refine_indicator=None, num_min_levels=0, num_max_levels=TMR.MAX_LEVEL):
        """
        ...
        """
        self.forest.refine(refine_indicator, min_lev=num_min_levels, max_lev=num_max_levels)
...
class pyTACSAdapt(BaseUI):
    ...
    """
    def __init__(self, comm, _initCallBack, _elemCallBack, _probCallBack, _adapt_output, **kwargs):
        """
        Constructor
    ...
    def setupModel(self, model_type, **kwargs):
        ...
        self.mesh_history[model.prob_name] = model.assembler.getVarsPerNode() * model.comm.allreduce(nnodes, op=MPI.SUM)
        return
    ...
    def interpolateField(self, field_type, writeSolution=False):
        ...
        TMR.computeInterpSolution(self.coarse.forest, self.coarse.assembler, self.fine.forest, self.fine.assembler, getattr(self.coarse, field_type), getattr(self.fine, field_type))

        # evaluate the fine-space output if the state is interpolated
        if field_type == 'state':
            self.fine.problem.setVariables(getattr(self.fine, field_type))
            self.fine.problem.evalFunctions(self.output_history, evalFuncs=[self.fine.output_name])

        # write out the selected field
        if writeSolution:
            # set the field in the fine problem
            self.fine.problem.setVariables(getattr(self.fine, field_type))
            self.fine.problem.writeSolution(baseName=f'{self.fine.prob_name}_{field_type}_interp')
    ...
    def reconstructField(self, field_type, compute_diff=False, writeSolution=False):
        ...
        TMR.computeReconSolution(self.coarse.forest, self.coarse.assembler, self.fine.forest, self.fine.assembler, getattr(self.coarse, field_type), getattr(self.fine, field_type), compute_diff=compute_diff)

        # write out the selected field
        if writeSolution:
            # set the field in the fine problem
            self.fine.problem.setVariables(getattr(self.fine, field_type))
            self.fine.problem.writeSolution(baseName=f'{self.fine.prob_name}_{field_type}_recon')
            # reset the state field in the fine problem
            self.fine.problem.setVariables(self.fine.state)
        return
    ...
    def estimateOutputError(self):
        ...
        error_estimate, output_correction, element_errors = TMR.adjointError(self.coarse.forest, self.coarse.assembler, self.fine.forest, self.fine.assembler, self.fine.state, self.fine.adjoint)

        # update histories
        self.output_history[f"{self.fine.prob_name}_{self.fine.output_name}"] += output_correction
    ...
    def adaptModelDecreasingThreshold(self, element_errors):
        ...
        refine_threshold = max(1., 2.**(self.num_decrease_iters - self.coarse.refine_iter))
        ...
        self.coarse.applyRefinement(refine.astype(np.intc), num_min_levels=self.num_min_ref_levels, num_max_levels=self.num_max_ref_levels)
        return
