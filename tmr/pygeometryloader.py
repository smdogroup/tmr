...
def model_method(method):
    ...
    def wrapped_method(self, *args, **kwargs):
        assert self.geom_model is not None, \
...
class pyGeometryLoader(BaseUI):
    def __init__(self, comm, geom_type, printDebug=False):
        ...
        assert geom_type in allowed_geom_types, \
    ...
    @model_method
    def nameGeometricEntities(self, vol_names=None, face_names=None, edge_names=None, vert_names=None, writeToTecplot=False):
        '''
    ...
    def setBoundaryConditions(self, bc_info):
        ...
        assert bc_type in valid_bc_types, \
        ...
        for name in bc_names:
            if name in entity_names:
                ...
                if self.comm.rank == 0:
                    self._user_warning(self.comm.rank, f'skipping boundary condition definition for `{name}`, label not found in model')
        return
    ...
    def setMeshOptions(self, **kwargs):
        ...
        '''
        allowed_keys = ('mesh_type_default',
                        'num_smoothing_steps',
        ...
        for key,val in kwargs.items():
            if key in allowed_keys: 
                ...
                if self.comm.rank == 0:
                    self._user_warning(self.comm.rank, f'skipping `{key}`, not a valid mesh option')
        return
    ...
    def setElementFeatureSize(self, feature_type, **kwargs):
        ...
        assert feature_type in allowed_feature_types, \
    ...
    def createTACSAssembler(self, elemCallBack, dvpn=1, **kwargs):
        ...
        self.assembler = self.creator.createTACS(forest=self.forest, comp_names=self.compDescripts, **kwargs)
    ...
    def getGlobalToLocalNodeIDDict(self):
        ...
        globalToLocalNodeMap = {gID:self.forest.getLocalNodeNumber(gID) for gID in global_ids}
    ...
    def getGlobalToLocalElemIDDict(self):
        ...
        globalToLocalElemMap = {(localID+offset):localID for localID in range(numLocElems)}
    ...
    def getElementComponents(self):
        ...
        if comps_not_found and self.comm.rank == 0:
            ...
            self._user_warning(self.comm.rank, f'the following {len(comps_not_found)} elements were not associated with any component defined in the model: {comps_not_found}')
        return compIDList
    ...
    def getGlobalElementIDsForComps(self, componentIDs):
        ...
        localToGlobalElemIDDict = {val:key for key,val in globalToLocalElemIDDict.items()}
    ...
    def _updateComps(self):
            ...
            if label is not None:
                if self.geom_type == 'oct':
                    if label in self.compIDtoLabel.values() and self.comm.rank == 0:
                        self._user_warning(self.comm.rank,f'component description `{label}` for ID {self.nComp} already exists')
        ...
        if comps_not_set and self.comm.rank == 0:
            self._user_warning(self.comm.rank, f'the following geometry labels are not valid component descriptions:\n\t{comps_not_set}')
        return
