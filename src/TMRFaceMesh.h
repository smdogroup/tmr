...
class TMRFaceMesh : public TMREntity {
 public:
  TMRFaceMesh(MPI_Comm _comm, TMRFace *face, TMRPoint *_X = NULL, int _npts = 0,
  ...
  TMRFaceMeshType getMeshType() { return mesh_type; }
  ...
  int getStructuredFaceIndex(TMREdge *e1, int idx1, TMREdge *e2, int idx2);
 ...
 private:
  ...
  void setPrescribedMesh(const TMRPoint *_X, int _npts, const int *_quads,
                         int _nquads);

  // Write the segments to a VTK file in parameter space
  void writeSegmentsToVTK(const char *filename, int npts, const double *params,
                          int nsegs, const int segs[]);

  // Print the triangle quality
  void printTriQuality(int ntris, const int tris[]);
  void writeTrisToVTK(const char *filename, int ntris, const int tris[]);

  // Write the dual mesh - used for recombination - to a file
  void writeDualToVTK(const char *filename, int nodes_per_elem, int nelems,
                      const int elems[], int num_dual_edges,
                      const int dual_edges[], const TMRPoint *p);
  ...
  void recombine(int ntris, const int tris[], const int tri_neighbors[],
                 ...
                 int num_edges, const int dual_edges[], int *_num_quads,
  ...
  int getRecombinedQuad(const int tris[], const int trineighbors[], int t1,
                        int t2, int quad[]);
  double computeRecombinedQuality(const int tris[], const int trineighbors[],
  ...
  void computeHolePts(const int nloops, int hole_pt, const int *loop_pt_offset,
  ...
  void createUnstructuredMesh(TMRMeshOptions options, TMRElementFeatureSize *fs,
                              TMRFaceMeshType mesh_type,
                              const int total_num_pts, const int nholes,
                              const double *params, const int nsegs,
                              const int *segments, const int num_degen,
                              const int *degen, int *npts, double **param_pts,
                              TMRPoint **Xpts, int *nquads, int **mesh_quads,
