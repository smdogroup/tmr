  ...
  if (fp){
    ...
    for ( int i = 0; i < num_nodes; i++ ){
      fprintf(fp, "%8s%16d%16d%16.9f%16.9f*%7d\n",
              "GRID*", i+1, coord_id,
              X[i].x, X[i].y, i+1);
      fprintf(fp, "*%7s%16.9f%16d%16s%16d        \n", 
	      " ", X[i].z, coord_disp, " ", seid);
    }
    ...
    for ( int i = 0, j = 0; i < num_faces; i++ ){
      ...
      // Loop over all possible edges in the surface mesh
      const int *quad;
      int nlocal = mesh->getLocalConnectivity(&quad);
      ...
      for ( int k = 0; k < nlocal; k++, j++ ){
        ...
        fprintf(fp, "%-8s%8d%8d%8d%8d%8d%8d%8d\n", "CQUADR", 
                j+1, part, quad[4*k]+1, quad[4*k+1]+1, 
                quad[4*k+2]+1, quad[4*k+3]+1, part);
      }
    }
