...
class TMRQuadForest : public TMREntity {
 public:
  ...
  // -----------------------------
  void setConnectivity(int _num_nodes, const int *_face_conn, int _num_faces);
  void setFullConnectivity(int _num_nodes, int _num_edges, int _num_faces,
                           const int *_face_conn, const int *_face_edge_conn);
  ...
  // ------------------------------------------
  void createInterpolation(TMRQuadForest *coarse, TACSBVecInterp *interp);
  ...
  TMRQuadrantArray *distributeQuadrants(TMRQuadrantArray *list,
                                        int use_tags = 0, int **quad_ptr = NULL,
                                        int **quad_recv_ptr = NULL,
                                        int include_local = 0,
                                        int use_node_index = 0);
  TMRQuadrantArray *sendQuadrants(TMRQuadrantArray *list, const int *quad_ptr,
 ...
 private:
  ...
  void matchQuadrantIntervals(TMRQuadrant *array, int size, int *ptr);
  void matchTagIntervals(TMRQuadrant *array, int size, int *ptr);

  // Balance-related routines
  // ------------------------
  // Balance the quadrant across the local tree and the forest
  void balanceQuadrant(TMRQuadrant *quad, TMRQuadrantHash *hash,
                       TMRQuadrantHash *ext_hash, TMRQuadrantQueue *queue,
                       const int balance_corner, const int balance_tree);

  // Add adjacent quadrants to the hashes/queues for balancing
  void addEdgeNeighbors(int edge_index, TMRQuadrant p, TMRQuadrantHash *hash,
                        TMRQuadrantHash *ext_hash, TMRQuadrantQueue *queue);
  void addCornerNeighbors(int corner, TMRQuadrant p, TMRQuadrantHash *hash,
                          TMRQuadrantHash *ext_hash, TMRQuadrantQueue *queue);

  // Add quadrants to adjacent non-owner processor queues
  void addAdjacentEdgeToQueue(int edge_index, TMRQuadrant p,
                              TMRQuadrantQueue *queue, TMRQuadrant orig);
  void addAdjacentCornerToQueue(int corner, TMRQuadrant p,
                                TMRQuadrantQueue *queue, TMRQuadrant orig);
  ...
  void createDependentConn(const int *node_nums, TMRQuadrantArray *nodes,
  ...
  int computeElemInterp(TMRQuadrant *node, TMRQuadForest *coarse,
  ...
  class TMRFaceConn : public TMREntity {
   public:
    ...
    ~TMRFaceConn() {
      ...
      if (face_edge_conn) {
        delete[] face_edge_conn;
      }
      if (node_face_ptr) {
        delete[] node_face_ptr;
      }
      if (node_face_conn) {
        delete[] node_face_conn;
      }
      if (edge_face_ptr) {
        delete[] edge_face_ptr;
      }
      if (edge_face_conn) {
        delete[] edge_face_conn;
      }
