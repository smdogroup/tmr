...
class TMROctForest : public TMREntity {
 public:
  ...
  // --------------------
  void setConnectivity(int _num_nodes, const int *_block_conn, int _num_blocks);
  void setFullConnectivity(int _num_nodes, int _num_edges, int _num_faces,
                           int _num_blocks, const int *_block_conn,
                           const int *_block_edge_conn,
                           const int *_block_face_conn);

  // Set/get the interpolation and order
  // -----------------------------------
  void setMeshOrder(int mesh_order, TMRInterpolationType interp_type =
  ...
  void createRandomTrees(int nrand = 10, int min_level = 0, int max_level = 8);
  ...
  // Refine the mesh
  // ---------------
  void refine(const int refinement[] = NULL, int min_level = 0,
  ...
  // ---------------------------------
  void getNodeConn(const int **_conn = NULL, int *_num_elements = NULL,
                   int *_num_owned_nodes = NULL, int *_num_local_nodes = NULL);
  int getDepNodeConn(const int **_ptr = NULL, const int **_conn = NULL,
                     const double **_weights = NULL);

  // Create interpolation/restriction operators
  // ------------------------------------------
  void createInterpolation(TMROctForest *coarse, TACSBVecInterp *interp);
  ...
  void evalInterp(const double pt[], double N[], double Nxi[], double Neta[],
                  double Nzeta[]);
  ...
  // -------------------------------------
  void getConnectivity(int *_nblocks, int *_nfaces, int *_nedges, int *_nnodes,
                       const int **_block_conn, const int **_block_face_conn,
                       const int **_block_edge_conn,
  ...
  void transformNode(TMROctant *oct, int edge_dir = -1,
                     int *edge_reversed = NULL, int *src_face_id = NULL);

  // Distribute the octant array in parallel to other processors
  // -----------------------------------------------------------
  TMROctantArray *distributeOctants(TMROctantArray *list, int use_tags = 0,
                                    int **oct_ptr = NULL,
  ...
  // -----------------------------------------------------------------------
  TMROctantArray *sendOctants(TMROctantArray *list, const int *oct_ptr,
                              const int *oct_recv_ptr, int use_node_index = 0);
 ...
 private:
  ...
  void matchOctantIntervals(TMROctant *array, int size, int *ptr);
  void matchTagIntervals(TMROctant *array, int size, int *ptr);

  // Balance-related routines
  // ------------------------
  // Balance the octant across the local tree and the forest
  void balanceOctant(TMROctant *oct, TMROctantHash *hash,
                     TMROctantHash *ext_hash, TMROctantQueue *queue,
                     const int balance_corner, const int balance_tree);

  // Add adjacent octants to the hashes/queues for balancing
  void addFaceNeighbors(int face_index, TMROctant p, TMROctantHash *hash,
                        TMROctantHash *ext_hash, TMROctantQueue *queue);
  void addEdgeNeighbors(int edge_index, TMROctant p, TMROctantHash *hash,
                        TMROctantHash *ext_hash, TMROctantQueue *queue);
  void addCornerNeighbors(int corner, TMROctant p, TMROctantHash *hash,
                          TMROctantHash *ext_hash, TMROctantQueue *queue);

  // Add octants to adjacent non-owner processor queues
  void addAdjacentFaceToQueue(int face_index, TMROctant p,
                              TMROctantQueue *queue, TMROctant orig);
  void addAdjacentEdgeToQueue(int edge_index, TMROctant p,
                              TMROctantQueue *queue, TMROctant orig);
  void addAdjacentCornerToQueue(int corner, TMROctant p, TMROctantQueue *queue,
                                TMROctant orig);
  ...
  void getEdgeNodes(TMROctant *oct, int edge_index, TMROctantArray *nodes,
                    const int *node_offset, int *edge_nodes);
  void getFaceNodes(TMROctant *oct, int face_index, TMROctantArray *nodes,
                    const int *node_offset, int *face_nodes);

  // Create the dependent node connectivity
  void createDependentConn(const int *node_nums, TMROctantArray *nodes,
  ...
  int computeElemInterp(TMROctant *node, TMROctForest *coarse, TMROctant *oct,
  ...
  class TMRBlockConn : public TMREntity {
   public:
    ...
    ~TMRBlockConn() {
      ...
      if (block_face_conn) {
        delete[] block_face_conn;
      }
      if (block_face_ids) {
        delete[] block_face_ids;
      }
      if (block_edge_conn) {
        delete[] block_edge_conn;
      }
      if (node_block_ptr) {
        delete[] node_block_ptr;
      }
      if (node_block_conn) {
        delete[] node_block_conn;
      }
      if (edge_block_ptr) {
        delete[] edge_block_ptr;
      }
      if (edge_block_conn) {
        delete[] edge_block_conn;
      }
      if (face_block_ptr) {
        delete[] face_block_ptr;
      }
      if (face_block_conn) {
        delete[] face_block_conn;
      }

      // Free the ownership data
      if (face_block_owners) {
        delete[] face_block_owners;
      }
      if (edge_block_owners) {
        delete[] edge_block_owners;
      }
      if (node_block_owners) {
