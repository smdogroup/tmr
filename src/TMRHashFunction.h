...
#define TMR_MIX_INT(a, b, c)                                                  \
  ((a -= c, a ^= TMR_ROT_INT(c, 4), c += b, b -= a, b ^= TMR_ROT_INT(a, 6),   \
    a += c, c -= b, c ^= TMR_ROT_INT(b, 8), b += a, a -= c,                   \
    a ^= TMR_ROT_INT(c, 16), c += b, b -= a, b ^= TMR_ROT_INT(a, 19), a += c, \
    c -= b, c ^= TMR_ROT_INT(b, 4), b += a))
#define TMR_FINAL_HASH(a, b, c)                                               \
  ((c ^= b, c -= TMR_ROT_INT(b, 14), a ^= c, a -= TMR_ROT_INT(c, 11), b ^= a, \
    b -= TMR_ROT_INT(a, 25), c ^= b, c -= TMR_ROT_INT(b, 16), a ^= c,         \
    a -= TMR_ROT_INT(c, 4), b ^= a, b -= TMR_ROT_INT(a, 14), c ^= b,          \
    c -= TMR_ROT_INT(b, 24)))
...
*/
inline uint32_t TMRIntegerFourTupleHash(uint32_t u, uint32_t v, uint32_t w,
                                        uint32_t x) {
  TMR_MIX_INT(u, v, w);
  u += x;
  return TMR_FINAL_HASH(u, v, w);
...
/*
  Create a hash value for four-tuples of unsigned integers
*/
inline uint32_t TMRIntegerFiveTupleHash(uint32_t u, uint32_t v, uint32_t w,
