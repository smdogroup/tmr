...
*/
const int face_to_edge_nodes[][2] = {{0, 2}, {1, 3}, {0, 1}, {2, 3}};
...
*/
void TMRQuadForest::freeMeshData(int free_quads, int free_owners) {
  if (free_quads) {
    if (quadrants) {
      delete quadrants;
    }
...
void TMRQuadForest::setTopology(TMRTopology *_topo) {
  ...
  if (_topo) {
    ...
    topo->getConnectivity(&_num_nodes, &_num_edges, &_num_faces, &_face_conn,
                          &_face_edge_conn);

    // Set the full connectivity
    setFullConnectivity(_num_nodes, _num_edges, _num_faces, _face_conn,
                        _face_edge_conn);
  }
}
...
*/
TMRTopology *TMRQuadForest::getTopology() { return topo; }
...
/*
  ...
  unstructured mesh.
*/
void TMRQuadForest::setConnectivity(int _num_nodes, const int *_face_conn,
                                    int _num_faces) {
...
*/
void TMRQuadForest::setFullConnectivity(int _num_nodes, int _num_edges,
                                        int _num_faces, const int *_face_conn,
...
void TMRQuadForest::computeEdgesToFaces() {
  ...
  for (int edge = 0; edge < num_edges; edge++) {
    ...
    int n1 =
        fdata->face_conn[4 * face_owner + face_to_edge_nodes[owner_index][0]];
    int n2 =
        fdata->face_conn[4 * face_owner + face_to_edge_nodes[owner_index][1]];
    ...
    for (int ip = fdata->edge_face_ptr[edge];
      ...
      for (int edge_index = 0; edge_index < 4; edge_index++) {
        int nn1 =
            fdata->face_conn[4 * face + face_to_edge_nodes[edge_index][0]];
        int nn2 =
            fdata->face_conn[4 * face + face_to_edge_nodes[edge_index][1]];
...
void TMRQuadForest::writeToTecplot(const char *filename) {
  ...
  if (mpi_rank == 0 && topo) {
    ...
    if (fp) {
      ...
      for (int k = 0; k < num_faces; k++) {
        fprintf(fp, "%d %d %d %d\n", face_conn[4 * k] + 1,
                face_conn[4 * k + 1] + 1, face_conn[4 * k + 3] + 1,
                face_conn[4 * k + 2] + 1);
      }
      fclose(fp);
    }
...
*/
int TMRQuadForest::getMeshOrder() { return mesh_order; }

/*
  Retrieve the interpolation type for this mesh
*/
TMRInterpolationType TMRQuadForest::getInterpType() { return interp_type; }
...
*/
int TMRQuadForest::getExtPreOffset(){
...
/*
  Evaluate the interpolant at the given parametric point
*/
void TMRQuadForest::evalInterp(const double pt[], double N[], double N1[],
                               double N2[]) {
  double Nu[MAX_ORDER], Nv[MAX_ORDER];
...
*/
void TMRQuadForest::evalInterp(const double pt[], double N[], double N1[],
                               double N2[], double N11[], double N22[],
                               double N12[]) {
...
/*
  ...
  nodes
*/
void TMRQuadForest::getConnectivity(int *_nfaces, int *_nedges, int *_nnodes,
                                    const int **_face_conn,
                                    const int **_face_edge_conn) {
  if (fdata) {
    if (_nfaces) {
      *_nfaces = fdata->num_faces;
    }
    if (_nedges) {
      *_nedges = fdata->num_edges;
    }
    if (_nnodes) {
      *_nnodes = fdata->num_nodes;
    }
    if (_face_conn) {
      *_face_conn = fdata->face_conn;
    }
    if (_face_edge_conn) {
      *_face_edge_conn = fdata->face_edge_conn;
    }
  } else {
    if (_nfaces) {
      *_nfaces = 0;
...
void TMRQuadForest::getInverseConnectivity(const int **_node_face_conn,
  ...
  if (fdata) {
    if (_node_face_conn) {
      *_node_face_conn = fdata->node_face_conn;
    }
    if (_node_face_ptr) {
      *_node_face_ptr = fdata->node_face_ptr;
    }
    if (_edge_face_conn) {
      *_edge_face_conn = fdata->edge_face_conn;
    }
    if (_edge_face_ptr) {
      *_edge_face_ptr = fdata->edge_face_ptr;
    }
  } else {
    if (_node_face_conn) {
      *_node_face_conn = NULL;
    }
    if (_node_face_ptr) {
      *_node_face_ptr = NULL;
    }
    if (_edge_face_conn) {
      *_edge_face_conn = NULL;
    }
    if (_edge_face_ptr) {
      *_edge_face_ptr = NULL;
    }
...
void TMRQuadForest::createTrees(int refine_level) {
  ...
  MPI_Allgather(&p, 1, TMRQuadrant_MPI_type, owners, 1, TMRQuadrant_MPI_type,
                comm);
...
/*
  Create a forest with the specified refinement level
*/
void TMRQuadForest::createRandomTrees(int nrand, int min_level, int max_level) {
  const int num_faces = fdata->num_faces;
...
void TMRQuadForest::repartition() {
  ...
  for (int i = 0; i < mpi_size; i++) {
    ...
    if (new_ptr[i + 1] >= ptr[mpi_rank] && new_ptr[i] < ptr[mpi_rank + 1]) {
      ...
      if (i == mpi_rank) {
        ...
        MPI_Isend(&array[start], count, TMRQuadrant_MPI_type, i, 0, comm,
  ...
  for (int i = 0; i < mpi_size; i++) {
    ...
    if (i != mpi_rank && ptr[i + 1] >= new_ptr[mpi_rank] &&
      ...
      // Send the element array to the new owner
      if (count > 0) {
        MPI_Recv(&new_array[start], count, TMRQuadrant_MPI_type, i, 0, comm,
                 MPI_STATUS_IGNORE);
      }
    }
  }
  ...
  owners = new TMRQuadrant[mpi_size];
  MPI_Allgather(&q, 1, TMRQuadrant_MPI_type, owners, 1, TMRQuadrant_MPI_type,
                comm);
...
TMRQuadForest *TMRQuadForest::coarsen() {
  ...
  if (fdata) {
    ...
    MPI_Allgather(&array[0], 1, TMRQuadrant_MPI_type, coarse->owners, 1,
                  TMRQuadrant_MPI_type, comm);
  }
...
*/
void TMRQuadForest::refine(const int refinement[], int min_level,
                           int max_level) {
...
int TMRQuadForest::getQuadrantMPIOwner(TMRQuadrant *quad) {
  int rank = 0;
  for (; (rank < mpi_size - 1 && owners[rank + 1].comparePosition(quad) <= 0);
       rank++)
    ;
...
*/
void TMRQuadForest::matchQuadrantIntervals(TMRQuadrant *array, int size,
                                           int *ptr) {
  ptr[0] = 0;
...
*/
void TMRQuadForest::matchTagIntervals(TMRQuadrant *array, int size, int *ptr) {
  ptr[0] = 0;
...
*/
TMRQuadrantArray *TMRQuadForest::distributeQuadrants(
    TMRQuadrantArray *list, int use_tags, int **_quad_ptr, int **_quad_recv_ptr,
    int include_local, int use_node_index) {
  ...
  MPI_Alltoall(quad_counts, 1, MPI_INT, quad_recv_counts, 1, MPI_INT, comm);
  ...
  TMRQuadrantArray *dist =
      sendQuadrants(list, quad_ptr, quad_recv_ptr, use_node_index);
...
TMRQuadrantArray *TMRQuadForest::sendQuadrants(TMRQuadrantArray *list,
  ...
  for (int i = 0, j = 0; i < mpi_size; i++) {
    if (i != mpi_rank && quad_ptr[i + 1] - quad_ptr[i] > 0) {
      ...
      MPI_Isend(&array[quad_ptr[i]], count, TMRQuadrant_MPI_type, i, 0, comm,
                &send_request[j]);
      j++;
    } else if (i == mpi_rank) {
      int count = quad_recv_ptr[i + 1] - quad_recv_ptr[i];
      if (count > 0 && (count == quad_ptr[i + 1] - quad_ptr[i])) {
        memcpy(&recv_array[quad_recv_ptr[i]], &array[quad_ptr[i]],
               count * sizeof(TMRQuadrant));
      }
    }
  }
  ...
  for (int i = 0; i < mpi_size; i++) {
    if (i != mpi_rank && quad_recv_ptr[i + 1] > quad_recv_ptr[i]) {
      ...
      MPI_Recv(&recv_array[quad_recv_ptr[i]], recv_count, TMRQuadrant_MPI_type,
...
*/
void TMRQuadForest::addEdgeNeighbors(int edge_index, TMRQuadrant p,
  ...
  for (int ip = fdata->edge_face_ptr[edge]; ip < fdata->edge_face_ptr[edge + 1];
       ip++) {
...
/*
  ...
  queue:   the array of newly added qudrants for each processor
*/
void TMRQuadForest::addCornerNeighbors(int corner, TMRQuadrant p,
                                       TMRQuadrantHash *hash,
                                       TMRQuadrantHash *ext_hash,
                                       TMRQuadrantQueue *queue) {
  // First determine the global edge number
  ...
  // Now, cycle through all the adjacent faces
  for (int ip = fdata->node_face_ptr[node]; ip < fdata->node_face_ptr[node + 1];
       ip++) {
...
*/
void TMRQuadForest::balanceQuadrant(TMRQuadrant *quad, TMRQuadrantHash *hash,
                                    TMRQuadrantHash *ext_hash,
                                    TMRQuadrantQueue *queue,
...
void TMRQuadForest::balance(int balance_corner) {
  if (!quadrants) {
    fprintf(stderr,
            "TMRQuadForest Error: Cannot call balance(), "
  ...
  for (int i = 0; i < quad_size; i++) {
    ...
    balanceQuadrant(&quad, hash, ext_hash, queue, balance_corner, balance_tree);
  }
  ...
  while (queue->length() > 0) {
    const int balance_tree = 1;
    TMRQuadrant quad = queue->pop();
    balanceQuadrant(&quad, hash, NULL, queue, balance_corner, balance_tree);
  }
...
*/
void TMRQuadForest::addAdjacentEdgeToQueue(int edge_index, TMRQuadrant p,
                                           TMRQuadrantQueue *queue,
                                           TMRQuadrant orig) {
  // First determine the global edge number
  int face = p.face;
...
*/
void TMRQuadForest::addAdjacentCornerToQueue(int corner, TMRQuadrant p,
                                             TMRQuadrantQueue *queue,
                                             TMRQuadrant orig) {
  // First determine the global edge number
  int face = p.face;
...
void TMRQuadForest::computeAdjacentQuadrants() {
  ...
  for (int i = 0; i < size; i++) {
    ...
    const int edge_ids[][2] = {{0, 2}, {1, 3}, {0, 1}, {2, 3}};
...
*/
void TMRQuadForest::computeAdjacentDepEdges(int edge_index, TMRQuadrant *b,
  ...
  int n1 = fdata->face_conn[4 * face_owner + face_to_edge_nodes[edge_index][0]];
  int n2 = fdata->face_conn[4 * face_owner + face_to_edge_nodes[edge_index][1]];
...
*/
void TMRQuadForest::transformNode(TMRQuadrant *quad, int *edge_reversed) {
...
void TMRQuadForest::createNodes() {
  if (!quadrants) {
    fprintf(stderr,
            "TMRQuadForest Error: Cannot call createNodes(), "
            "no quadrants have been created\n");
    return;
  }
  if (conn) {
  ...
  TMRQuadrantArray *dist_nodes =
      distributeQuadrants(ext_array, use_tags, &send_ptr, &recv_ptr);
  delete ext_array;
  ...
  TMRQuadrantArray *return_nodes =
      sendQuadrants(dist_nodes, recv_ptr, send_ptr);
  delete dist_nodes;
  delete[] recv_ptr;
  delete[] send_ptr;
...
TMRQuadrantArray *TMRQuadForest::createLocalNodes() {
  ...
  int *send_ptr, *recv_ptr;
  TMRQuadrantArray *recv_nodes = distributeQuadrants(
    ...
    for (int i = 0; i < num_elements; i++) {
      ...
      for (int jj = 1; jj < mesh_order - 1; jj++) {
        for (int ii = 1; ii < mesh_order - 1; ii++) {
          ...
          c[offset] =
              node_offset[index] + (ii - 1) + (jj - 1) * (mesh_order - 2);
        }
      }
    }
  }
  ...
  if (topo && (interp_type == TMR_BERNSTEIN_POINTS && mesh_order > 2)) {
      ...
      for (int jj = 0; jj < mesh_order; jj++) {
        for (int ii = 0; ii < mesh_order; ii++) {
          ...
          surf->evalPoint(u + 0.5 * d * (1.0 + knots[ii]),
                          v + 0.5 * d * (1.0 + knots[jj]), &Xtmp[local_index]);
        }
      }
    ...
    for (int i = 0; i < num_elements; i++) {
      ...
      for (int jj = 0; jj < mesh_order; jj++) {
        for (int ii = 0; ii < mesh_order; ii++) {
          // Compute the mesh index
          int node = conn[mesh_order * mesh_order * i + ii + jj * mesh_order];
          int index = getLocalNodeNumber(node);
          if (!flags[index]) {
            flags[index] = 1;
            surf->evalPoint(u + 0.5 * d * (1.0 + knots[ii]),
                            v + 0.5 * d * (1.0 + knots[jj]), &X[index]);
          }
        }
      }
    }
...
*/
void TMRQuadForest::getNodeConn(const int **_conn, int *_num_elements,
...
TMRQuadrantArray *TMRQuadForest::getQuadsWithName(const char *name) {
  if (!topo) {
    fprintf(stderr,
            "TMRQuadForest Error: Must define topology to use "
            "getQuadsWithName()\n");
    return NULL;
  }
  if (!quadrants) {
    fprintf(stderr,
            "TMRQuadForest Error: Must create quadrants to use "
            "getQuadsWithName()\n");
    return NULL;
  }
  ...
  for (int i = 0; i < size; i++) {
    ...
    if ((!name && !face_name) || (face_name && strcmp(face_name, name) == 0)) {
      queue->push(&array[i]);
    } else {
...
*/
int TMRQuadForest::getNodesWithName(const char *name, int **_nodes) {
  if (!topo) {
    fprintf(stderr,
            "TMRQuadForest Error: Must define topology to use "
            "getNodesWithName()\n");
  ...
  if (!conn) {
    fprintf(stderr,
            "TMRQuadForest Error: Nodes must be created before "
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (fx || fy) {
      ...
      for (int ii = 0; ii < nedges; ii++) {
        TMREdge *edge;
        int edge_num =
            fdata->face_edge_conn[4 * quads[i].face + edge_index[ii]];
        topo->getEdge(edge_num, &edge);
        const char *edge_name = edge->getName();
        if (edge_name && strcmp(edge_name, name) == 0) {
          for (int k = 0; k < mesh_order; k++) {
...
*/
TMRQuadrant *TMRQuadForest::findEnclosing(const int order, const double *knots,
                                          TMRQuadrant *node, int *mpi_owner) {
...
*/
int TMRQuadForest::computeElemInterp(TMRQuadrant *node, TMRQuadForest *coarse,
                                     TMRQuadrant *quad, TMRIndexWeight *weights,
                                     double *tmp) {
  // Loop over the array of nodes
  const int coarse_nodes_per_element = coarse->mesh_order * coarse->mesh_order;
  ...
  if (interp_type != coarse->interp_type) {
    fprintf(stderr,
            "TMRQuadForest Error: Interpolation types between "
  ...
  if (interp_type == TMR_BERNSTEIN_POINTS &&
    ...
    fprintf(stderr,
            "TMRQuadForest Error: mesh order difference across "
            "grids should be 1\n");
  }
  ...
  if (interp_type == TMR_BERNSTEIN_POINTS) {
    ...
    if ((i == 0 && quad->x == node->x) ||
      ...
      if (mesh_order == coarse->mesh_order) {
        double u =
            -1.0 +
            2.0 * (node->x + 0.5 * h * (1.0 + bern_knots[i]) - quad->x) / hc;
    ...
    if ((j == 0 && quad->y == node->y) ||
      ...
      if (mesh_order == coarse->mesh_order) {
        double v =
            -1.0 +
            2.0 * (node->y + 0.5 * h * (1.0 + bern_knots[j]) - quad->y) / hc;
    ...
    if ((i == 0 && quad->x == node->x) ||
      ...
      double u =
          -1.0 +
          2.0 * (node->x + 0.5 * h * (1.0 + interp_knots[i]) - quad->x) / hc;
    ...
    if ((j == 0 && quad->y == node->y) ||
      ...
      double v =
          -1.0 +
          2.0 * (node->y + 0.5 * h * (1.0 + interp_knots[j]) - quad->y) / hc;
...
void TMRQuadForest::createInterpolation(TMRQuadForest *coarse,
  ...
  for (int i = 0; i < recv_size; i++) {
    TMRQuadrant *t = coarse->findEnclosing(mesh_order, knots, &recv_nodes[i]);
    if (t) {
      // Compute the element interpolation
      ...
      fprintf(stderr,
              "[%d] TMRQuadForest Error: Destination processor does "
              "not own node\n",
              mpi_rank);
    }
  }
