...
*/
void bspline_knots(int n, int k, double umin, double umax, double *T) {
...
*/
TMRBsplineCurve::TMRBsplineCurve(int _n, int _k, const TMRPoint *_pts) {
  nctl = _n;
  ...
  if (ku > MAX_BSPLINE_ORDER) {
    ku = MAX_BSPLINE_ORDER;
  }
...
*/
TMRBsplineCurve::TMRBsplineCurve(int _n, int _k, const double *_Tu,
                                 const TMRPoint *_pts) {
  nctl = _n;
...
*/
int TMRBsplineCurve::eval2ndDeriv(double t, TMRPoint *X, TMRPoint *Xt,
                                  TMRPoint *Xtt) {
  double Nu[2 * MAX_BSPLINE_ORDER];
  double work[2 * MAX_BSPLINE_ORDER + MAX_BSPLINE_ORDER * MAX_BSPLINE_ORDER];
...
*/
void TMRBsplineCurve::split(double tsplit, TMRBsplineCurve **c1,
                            TMRBsplineCurve **c2) {
  ...
  for (int i = 0; i < nctl + ku; i++) {
    ...
    if (Tu[i] <= tsplit + eps_u && Tu[i] + eps_u >= tsplit) {
      insert--;
    }
  }
...
*/
TMRBsplineCurve *TMRBsplineCurve::refineKnots(const double *_Tnew, int nnew) {
...
/*
  Create a B-spline curve with a uniform knot vector
*/
TMRBsplinePcurve::TMRBsplinePcurve(int _n, int _k, const double *_pts) {
  nctl = _n;
...
/*
  ...
  pts:  the TMRPoint interpolation points
*/
TMRBsplinePcurve::TMRBsplinePcurve(int _n, int _k, const double *_Tu,
                                   const double *_pts) {
  nctl = _n;
...
*/
TMRBsplinePcurve::TMRBsplinePcurve(int _n, int _k, const double *_Tu,
                                   const double *_wts, const double *_pts) {
  nctl = _n;
...
/*
  Given the parametric point, evaluate the derivative
*/
int TMRBsplinePcurve::evalDeriv(double t, double *_u, double *_v, double *_ut,
                                double *_vt) {
  double Nu[2 * MAX_BSPLINE_ORDER];
  double work[2 * MAX_BSPLINE_ORDER + MAX_BSPLINE_ORDER * MAX_BSPLINE_ORDER];
...
/*
  Given the parametric point, evaluate the second derivative
*/
int TMRBsplinePcurve::eval2ndDeriv(double t, double *_u, double *_v,
                                   double *_ut, double *_vt, double *_utt,
                                   double *_vtt) {
  double Nu[2 * MAX_BSPLINE_ORDER];
  double work[2 * MAX_BSPLINE_ORDER + MAX_BSPLINE_ORDER * MAX_BSPLINE_ORDER];
...
/*
  ...
  knots at the specified locations.
*/
TMRBsplinePcurve *TMRBsplinePcurve::refineKnots(const double *_Tnew, int nnew) {
  // Allocate the new vectors
  int kbar = ku;
  int nbar = nctl + nnew;
  double *Tbar = new double[nbar + kbar];
    ...
    for (int l = 1; l < ku; l++) {
      ...
      if (alpha == 0.0) {
        ...
        if (wts) {
          ...
          pbar[2 * (ind - 1) + 1] =
              (alpha * wbar[ind - 1] * pbar[2 * (ind - 1) + 1] +
               (1.0 - alpha) * wbar[ind] * pbar[2 * ind]);
          wbar[ind - 1] = (alpha * wbar[ind - 1] + (1.0 - alpha) * wbar[ind]);
        } else {
          pbar[2 * (ind - 1)] =
              alpha * pbar[2 * (ind - 1)] + (1.0 - alpha) * pbar[2 * ind];
          pbar[2 * (ind - 1) + 1] = alpha * pbar[2 * (ind - 1) + 1] +
                                    (1.0 - alpha) * pbar[2 * ind + 1];
        }
      }
    }
...
*/
TMRBsplineSurface::TMRBsplineSurface(int _nu, int _nv, int _ku, int _kv,
                                     const TMRPoint *_pts) {
  ...
  if (kv > MAX_BSPLINE_ORDER) {
    kv = MAX_BSPLINE_ORDER;
  }
...
*/
void TMRBsplineSurface::getRange(double *umin, double *vmin, double *umax,
                                 double *vmax) {
  *umin = Tu[0];
  *umax = Tu[nu + ku - 1];
  *vmin = Tv[0];
...
*/
int TMRBsplineSurface::invEvalPoint(TMRPoint point, double *uf, double *vf) {
  // Did this evaluation fail or not?
  int fail = 1;
...
*/
int TMRBsplineSurface::evalDeriv(double u, double v, TMRPoint *X, TMRPoint *Xu,
                                 TMRPoint *Xv) {
  // The basis functions/work arrays
...
*/
int TMRBsplineSurface::eval2ndDeriv(double u, double v, TMRPoint *X,
                                    TMRPoint *Xu, TMRPoint *Xv, TMRPoint *Xuu,
                                    TMRPoint *Xuv, TMRPoint *Xvv) {
  // The basis functions/work arrays
  double Nu[2 * MAX_BSPLINE_ORDER], Nv[2 * MAX_BSPLINE_ORDER];
  double work[2 * MAX_BSPLINE_ORDER + MAX_BSPLINE_ORDER * MAX_BSPLINE_ORDER];
...
*/
TMRCurveInterpolation::~TMRCurveInterpolation() { delete[] interp; }
...
TMRBsplineCurve *TMRCurveInterpolation::createCurve(int ku) {
  ...
  if (nctl < ninterp) {
    ...
    int info = 0;
    TmrLAPACKdgbsv(&nctl, &upp_bnd, &low_bnd, &nrhs, A, &lda, ipiv, rhs, &nctl,
                   &info);
...
*/
TMRCurveLofter::TMRCurveLofter(TMRBsplineCurve **_curves, int _num_curves) {
...
TMRBsplineSurface *TMRCurveLofter::createSurface(int kv) {
  ...
  for (int i = 0; i < num_curves; i++) {
    ...
    while (k < (nu + ku) && (curve_Tu[ptr[i]] <= curve_Tu[k] + eps_u &&
  ...
  for (int i = 0; i < num_curves; i++) {
    ...
    for (int j = 0, k = 0; j < len; j++) {
      if (Tu[j] <= curve_Tu[k] + eps_u && Tu[j] + eps_u >= curve_Tu[k]) {
        k++;
      } else {
        Tnew[nnew] = Tu[j];
        nnew++;
