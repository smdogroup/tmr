...
void TMRTopoProblem::addFrequencyConstraint( double sigma,
  ...
  if (!freq){
    ...
    if (use_jd){
      ...
      freq = new TACSFrequencyAnalysis(assembler, sigma, mmat,
                                       kmat, pcmat, pc,
...
void TMRTopoProblem::addBucklingConstraint( double sigma,
  ...
  if (!buck){
    ...
    for ( int i = 0; i < num_load_cases; i++ ){
      ...
      buck[i] = new TACSLinearBuckling(assembler, sigma, gmat, kmat,
...
*/
void TMRTopoProblem::addQnCorrectionCallback( int ncon,
                                              void *callback_ptr,
...
int TMRTopoProblem::evalObjConGradient( ParOptVec *xvec,
  ...
  if (wrap){
    ...
    // else if (!objectiveGradientCallback) { // For compliance objective
      for ( int i = 0; i < num_load_cases; i++ ){
        assembler->setVariables(vars[i]);
...
// -----------------------------------------------
void TMRTopoProblem::computeQuasiNewtonUpdateCorrection( ParOptVec *x,
                                                         ParOptScalar *z,
                                                         ParOptVec *zw,
                                                         ParOptVec *s,
                                                         ParOptVec *y ){
  ...
  */
  if (use_qn_correction_comp_obj){
    ...
    ParOptBVecWrap *proj_deriv_wrap = dynamic_cast<ParOptBVecWrap*>(proj_deriv);
    ...
    if (proj_deriv_wrap) {
      for ( int i = 0; i < num_load_cases; i++ ){
        // assembler->setVariables(vars[i]);
        assembler->addAdjointResProducts(-obj_weights[i], 1, &vars[i], &proj_deriv_wrap->vec);
      }
    }

    double gx = proj_deriv->norm();
    if (rank == 0)
      printf("Qn correction: norm of g(x)    = %.5e\n", gx);
    ...
    if (rank == 0)
      printf("Qn correction: norm of dg(x)   = %.5e\n", dgx);
    ...
    if (rank == 0)
      printf("Qn correction: norm of dg(x)/dh= %.5e\n", dgxdh);

    // Apply filter transpose to projected derivative
    filter->addValues(proj_deriv_wrap->vec);
    // filter->applyTranspose(proj_deriv_wrap->vec, proj_deriv_wrap->vec);  // Same as addValues()
...
void TMRTopoProblem::writeOutput( int iter, ParOptVec *xvec ){
  ...
  if (prefix){
    ...
    if ((buck || freq) && f5_eigen_frequency > 0 &&
      ...
      char *filename = new char[ strlen(prefix) + 100 ];
      TACSToFH5 *f5 = new TACSToFH5(assembler, f5_eigen_element_type,
                                    f5_eigen_write_flag);
      f5->incref();
