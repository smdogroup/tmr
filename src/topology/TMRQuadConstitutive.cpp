...
*/
int TMRQuadConstitutive::getDesignVarNums(int elemIndex, int dvLen,
                                          int dvNums[]) {
  const int order = forest->getMeshOrder();
...
/*
  ...
  the density
*/
int TMRQuadConstitutive::setDesignVars(int elemIndex, int dvLen,
...
*/
int TMRQuadConstitutive::getDesignVars(int elemIndex, int dvLen,
                                       TacsScalar dvs[]) {
  const int order = forest->getMeshOrder();
  const int len = order * order;
...
*/
int TMRQuadConstitutive::getDesignVarRange(int elemIndex, int dvLen,
...
*/
TacsScalar TMRQuadConstitutive::evalDensity(int elemIndex, const double pt[],
                                            const TacsScalar X[]) {
  const int order = forest->getMeshOrder();
  const int len = order * order;
...
// Add the derivative of the density
void TMRQuadConstitutive::addMassMatrixDensityDVSens(
...
*/
void TMRQuadConstitutive::evalStress(int elemIndex, const double pt[],
...
*/
void TMRQuadConstitutive::evalTangentStiffness(int elemIndex, const double pt[],
                                               const TacsScalar X[],
                                               TacsScalar C[]) {
  const int order = forest->getMeshOrder();
  const int len = order * order;
...
*/
void TMRQuadConstitutive::addStressDVSens(int elemIndex, TacsScalar scale,
                                          const double pt[],
                                          const TacsScalar X[],
                                          const TacsScalar e[],
  ...
  for (int j = 0; j < nmats; j++) {
    ...
    TacsScalar product =
        scale * dpenalty * (s[0] * psi[0] + s[1] * psi[1] + s[2] * psi[2]);
...
/*
  Evaluate the tangent stiffness matrix
*/
void TMRQuadConstitutive::evalGeometricTangentStiffness(int elemIndex,
                                                        const double pt[],
                                                        const TacsScalar X[],
                                                        TacsScalar C[]) {
  const int order = forest->getMeshOrder();
...
/*
  ...
  times alpha to the design variable array dvSens
*/
void TMRQuadConstitutive::addGeometricTangentStressDVSens(
...
void TMRQuadConstitutive::evalThermalStrain(int elemIndex, const double pt[],
                                            const TacsScalar X[],
...
void TMRQuadConstitutive::addThermalStrainDVSens(
...
void TMRQuadConstitutive::evalHeatFlux(int elemIndex, const double pt[],
                                       const TacsScalar X[],
...
void TMRQuadConstitutive::evalTangentHeatFlux(int elemIndex, const double pt[],
                                              const TacsScalar X[],
                                              TacsScalar C[]) {
  const int order = forest->getMeshOrder();
  const int len = order * order;
...
void TMRQuadConstitutive::addHeatFluxDVSens(int elemIndex, TacsScalar scale,
                                            const double pt[],
                                            const TacsScalar X[],
                                            const TacsScalar grad[],
                                            const TacsScalar psi[], int dvLen,
  ...
  for (int j = 0; j < nmats; j++) {
    ...
    TacsScalar product =
        scale * dpenalty * (flux[0] * psi[0] + flux[1] * psi[1]);
...
TacsScalar TMRQuadConstitutive::evalFailure(int elemIndex, const double pt[],
                                            const TacsScalar X[],
                                            const TacsScalar e[]) {
  const int order = forest->getMeshOrder();
  const int len = order * order;
...
/*
  ...
  variables
*/
void TMRQuadConstitutive::addFailureDVSens(int elemIndex, TacsScalar scale,
                                           const double pt[],
                                           const TacsScalar X[],
...
TacsScalar TMRQuadConstitutive::evalDesignFieldValue(int elemIndex,
                                                     const double pt[],
                                                     const TacsScalar X[],
                                                     int index) {
