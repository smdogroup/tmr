...
void TMRCoupledThermoQuadStiffness::heatflux( const double pt[], 
  ...
  if (nvars == 1){
    ...
    *qn = r_factor*(normal[0]*q[0] + normal[1]*q[1]);
  }
  else {
    *qn = 0.0;
    for ( int j = 1; j < nvars; j++ ){
...
// Evaluate the failure criteria w.r.t. design variables
void TMRCoupledThermoQuadStiffness::addHeatFluxDVSens( const double pt[], 
                                                       const TacsScalar normal[],
                                                       const TacsScalar e[],
                                                       TacsScalar alpha,
                                                       TacsScalar dvSens[], 
...
void TMRCoupledThermoQuadStiffness::heatfluxStrainSens( const double pt[], 
                                                        const TacsScalar normal[],
                                                        TacsScalar sens[] ){
  const double eps = props->eps;
  if (filter && dv){
