...
def getShapeFuncs(xi, eta):
    ...
    nxi = np.array([-0.5*xi*(1.0-xi),
                     (1.0-xi)*(1.0+xi),
                     0.5*xi*(1.0+xi)])
    neta = np.array([-0.5*eta*(1.0-eta),
                     (1.0-eta)*(1.0+eta),
                     0.5*eta*(1.0+eta)])
...
def getEnrichmentFuncs(xi, eta):
    ...
    N = np.array([cxi, cxi*eta, cxi*eta**2,
                  ceta, ceta*xi, ceta*xi**2, cxi*ceta])

    # Compute the derivative of the enriched shape functions
    Nxi = np.array([dxi, dxi*eta, dxi*eta**2,
                    0.0, ceta, 2.0*ceta*xi, dxi*ceta])
    Neta = np.array([0.0, cxi, 2.0*cxi*eta, 
                     deta, deta*xi, deta*xi**2, cxi*deta])
...
def computeElemError(uvals, ubar, Xpts, nquad=5):
    ...
    if nquad == 5:
        quadpts = [ -0.906179845938664, 
                     -0.538469310105683, 
                     0.0,               
                     0.538469310105683, 
                     0.906179845938664 ]
        quadwts = [ 0.236926885056189,  
                    0.478628670499366, 
                    0.568888888888889, 
                    0.478628670499366, 
                    0.236926885056189 ]
        ...
        nquad = 6
        quadpts = [-0.9324695142031520278123016, 
                    -0.6612093864662645136613996, 
                    -0.2386191860831969086305017, 
                    0.2386191860831969086305017, 
                    0.6612093864662645136613996, 
                    0.9324695142031520278123016 ]
        quadwts = [ 0.1713244923791703450402961,  
...
globalXpts = 2.0*np.array([0.25, 0.25,
                           0.5, 0.35,
                           1.0, 0.4,
                           0.15, 0.35,
                           0.45, 0.45,
