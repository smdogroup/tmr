...
class ksFSDTStiffness : public FSDTStiffness {
 public:
  ...
  ksFSDTStiffness( TacsScalar _ksweight, TacsScalar _rho, 
  ...
  void getDesignVarRange( TacsScalar lowerBound[], 
                          TacsScalar upperBound[], int dvLen ){
    if (tNum >= 0 && tNum < dvLen){
  ...
  void failure( const double pt[], 
    ...
    TacsScalar fmax = (TacsRealPart(failTop) > TacsRealPart(failBot) ? 
                       failTop : failBot);

    // Compute the aggregate of the top/bottom
    *fail = fmax + log(exp(ksweight*(failTop - fmax)) + 
                       exp(ksweight*(failBot - fmax)))/ksweight;
  }
  ...
  void addFailureDVSens( const double pt[],
    ...
    if (tNum >= 0 && tNum < dvLen){
      ...
      VonMisesFailurePlaneStressSens(stressSens, stress, 
                                     yieldStress);
      fdvSens[tNum] += 
        -0.5*botExp*(calculatePlaneStressTSensProduct(stressSens, strain));
    }
  }
