...
class Trussbraced(PGMconfiguration):
    def _define_comps(self):
        ...
        self.comps["lstrut"] = PGMwing(
            num_x=4, num_z=4, left_closed=True, right_closed=True
        )
        ...
        self.comps["lwing_fuse"] = PGMjunction(
            self, "fuse", "lft", "E", [0, 1], "lwing", "right", fweight=4, mweight=2
        )
        self.comps["lstrut_fuse"] = PGMjunction(
            self, "fuse", "lft", "E", [4, 2], "lstrut", "right"
        )
        self.comps["lstrut_lwing"] = PGMjunction(
            self, "lwing", "low", "S", [4, 1], "lstrut", "left", fweight=3, mweight=3
        )
        self.comps["lv_lwing"] = PGMjunction(
            self, "lwing", "low", "S", [1, 3], "lv", "left"
        )
        self.comps["lv_lstrut"] = PGMjunction(
            self, "lstrut", "upp", "S", [1, 0], "lv", "right"
        )
        self.comps["vtail_fuse"] = PGMjunction(
            self, "fuse", "top", "E", [1, 10], "vtail", "right"
        )
        self.comps["ltail_vtail"] = PGMjunction(
            self, "vtail", "low", "N", [0, 1], "ltail", "right"
        )
    ...
    def _define_params(self):
        ...
        fuse["scl"].params["rad1"] = PGMparameter(
            4, 1, order_u=4, pos_u=[0, 0.01, 0.05, 0.12]
        )

        fuse["scl"].params["rad2"] = PGMparameter(2, 1, pos_u=[0.12, 0.76])
        fuse["scl"].params["rad3"] = PGMparameter(
            4, 1, order_u=4, pos_u=[0.76, 0.83, 0.99, 1]
        )
        fuse["scl"].params["tail"] = PGMparameter(2, 3, pos_u=[0.76, 1.0])
        fuse["flt"].params["flt1a"] = PGMparameter(
            4, 2, order_u=4, pos_u=[0.24, 0.27, 0.33, 0.36], pos_v=[0.5, 1]
        )
        fuse["flt"].params["flt1b"] = PGMparameter(
            2, 2, pos_u=[0.36, 0.41], pos_v=[0.5, 1]
        )
        fuse["flt"].params["flt1c"] = PGMparameter(
            4, 2, order_u=4, pos_u=[0.41, 0.44, 0.49, 0.52], pos_v=[0.5, 1]
        )
        fuse["flt"].params["flt2a"] = PGMparameter(
            4, 2, order_u=4, pos_u=[0.24, 0.27, 0.33, 0.36], pos_v=[0, 0.5]
        )
        fuse["flt"].params["flt2b"] = PGMparameter(
            2, 2, pos_u=[0.36, 0.41], pos_v=[0, 0.5]
        )
        fuse["flt"].params["flt2c"] = PGMparameter(
            4, 2, order_u=4, pos_u=[0.41, 0.44, 0.49, 0.52], pos_v=[0, 0.5]
        )
...
def geomach_to_tmr(bse):
    ...
    for i in range(nfaces):
        for ii in range(4):
            ...
            if v1 == surf_ptrs[i, i1, j1] - 1 and v2 == surf_ptrs[i, i2, j2] - 1:
                vert1 = verts[v1]
                vert2 = verts[v2]
                pts = np.array(
            ...
            elif v2 == surf_ptrs[i, i1, j1] - 1 and v1 == surf_ptrs[i, i2, j2] - 1:
                vert1 = verts[v2]
                vert2 = verts[v1]
                pts = np.array(
