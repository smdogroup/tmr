...
def makePlateGeom(width=2.0, height=1.0, npanels=2, makeIGES=False):
    '''
    ...
    for iface in range(nfaces):
        i = iface % 4
        j = iface // 4
        quad_conn.append([i+nx*j, (i+1)+nx*j, (i+1)+nx*(j+1), i+nx*(j+1)])
    ...
    for i in range(nedges):
        ...
        lines.append(ctx.makeGeometry(egads.CURVE, mtype=egads.LINE, rdata=[coords[n1_ind], delta]))
    ...
    for i in range(nedges):
        ...
        edges.append(ctx.makeTopology(egads.EDGE, mtype=egads.TWONODE, geom=lines[i], children=[nodes[n1_ind], nodes[n2_ind]], rdata=[0, dist]))
    ...
    for i in range(nfaces):
        ...
        eloop, nloop_edges = ctx.makeLoop([edges[e1_ind], edges[e2_ind], edges[e3_ind], edges[e4_ind]])
