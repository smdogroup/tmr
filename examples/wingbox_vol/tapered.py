...
def createTopoProblem(forest, order=2, nlevels=2):
    ...
    assemblers.append(creator.createTACS(order, forest)) # seg faults here "gloabal name 'elements' not defined in tmr.TMR._createOctTopoElement"
...
mesh.writeToVTK('tapered-mesh.vtk', 'hex')
...
problem.initialize()
problem.setPrefix('./results_tapered')

max_bfgs = 20
opt = ParOpt.pyParOpt(problem, max_bfgs, ParOpt.BFGS)
opt.setOutputFrequency(1)
opt.setOutputFile('results_tapered/opt.dat')
