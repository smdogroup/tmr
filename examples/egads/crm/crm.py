...
def create_faces(ctx, X, frame_edges, nodes=None, edges=None, faces=None):
    ...
    for index, e in enumerate(frame_edges):
        if edges[index + ne] is None:
            ...
            edge = ctx.makeTopology(
                ...
                topo_type,
                geom=line,
                children=[nodes[e[0] + nx], nodes[e[1] + nx]],
                rdata=[0, np.sqrt(np.dot(d, d))],
            )
    ...
    for i, e in enumerate(frame_edges):
        if faces[i] is None:
            ...
            senses = [1, 1, -1, -1]
            loop = ctx.makeTopology(
                egads.LOOP, egads.CLOSED, children=elist, sens=senses
            )
...
def compute_ribspar_edges(leList, teList, nrib1=5, nrib2=44):
    ...
    dspar = np.array([teList[2, 0] - teList[1, 0], teList[2, 1] - teList[1, 1]])
    ...
    droot = np.array([teList[1, 0] - leList[1, 0], teList[1, 1] - leList[1, 1]])

    # Create the connectivity
    conn = [[0, 1, 4, 3], [1, 2, 5, 4]]
    X = [
        [teList[0, 0], teList[0, 1]],
...
# Scale the positions
leList = np.array([[670.5, 0.0], [670.5, 77.5], [1173.25, 745.0]])
teList = np.array([[821.0, 0.0], [821.0, 77.5], [1190.5, 745.0]])
...
x, edge_conn, face_conn, face_sens = compute_ribspar_edges(
    leList, teList, nrib1=5, nrib2=43
)
...
ribspar_shell = ctx.makeTopology(egads.SHELL, egads.OPEN, children=ribspar_faces)
ribspar_body = ctx.makeTopology(egads.BODY, egads.SHEETBODY, children=[ribspar_shell])
