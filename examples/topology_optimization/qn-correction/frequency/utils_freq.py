    ...
    def constraint(self, fltr, mg):
        ...
        if self.fltr is None:
            ...
            if self.eig_method == "jd":
                ...
                self.jd.setTolerances(
                    eig_rtol=1e-6, eig_atol=1e-6, rtol=1e-6, atol=1e-12
                )
            ...
            # Set up Shift-and-invert Lanczos method
                ...
                is_flexible = 0
                self.ksm_Amat = (
                    self.assembler.createMat()
                )  # We need a separate matrix to apply shift
                ksm = TACS.KSM(
                    self.ksm_Amat, self.mg, gmres_iters, nrestart, is_flexible
                )
        ...
        if self.eig_method == "jd":
            ...
            if self.comm.rank == 0:
                print(
                    "[JD Recycle] JD solver is recycling {:d} eigenvectors...".format(
                        num_recycle
                    )
                )
