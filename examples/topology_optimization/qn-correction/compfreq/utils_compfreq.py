...
def create_problem(
    ...
    density=2600.0,
    iter_offset=0,
    qn_correction_comp=True,
    qn_correction_freq=True,
    comp_scale=1.0,
    eig_scale=1.0,
    eq_constr=False,
    max_jd_size=100,
    ...
    problem.addConstraintCallback(
        nconstr, nineq, constr_callback_fun, constr_callback_grad
    )
    ...
    if qn_correction_freq and not qn_correction_comp:
        if has_freq_constr:
            ...
            print(
                "[Warning]doesn't have frequency constraint, \
