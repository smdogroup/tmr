        ...
        for optimizer in optimizers:
            ...
            if not os.path.isfile(os.path.join(case_folder, pkl_name)):
                cmd = "python {:s} {:s} {:s} --prefix {:s}".format(
                    ...
                    optimizer2cmd(
                        optimizer, problem, compfreq_qn
                    ),  # --optimizer paropt --qn-correction
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--compfreq-qn", type=str, default="comp", choices=["comp", "freq", "compfreq"]
    )
    p.add_argument(
        "--optimizer",
        type=str,
    ...
    case_cmds, n_exist_case = createCaseCmds(
        args.problem, physics, args.optimizer, exescript, args.compfreq_qn
    )
    ...
    with open(pbs, "w") as f:
        f.write("#!/bin/bash\n")
        f.write(
            "#SBATCH --job-name={:s}-n{:d}-{:d}\n".format(
                args.problem, args.start, args.end
            )
        )
        ...
        f.write(
            "export casecmd=`cat {:s} | head -$SLURM_ARRAY_TASK_ID | tail -1`\n".format(
                output
            )
        )
        f.write("\n")
