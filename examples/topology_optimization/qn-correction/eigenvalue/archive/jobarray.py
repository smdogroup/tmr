...
if __name__ == "__main__":
    ...
    create_models_exec = "python {:s} --domain {:s} --AR {:s} --ratio {:s}".format(
        args.create_models_exec,
        list2str(egads_domains),
        list2str(args.AR),
        list2str(args.ratio),
    )
    ...
    commons = (
        "--r0-frac {:f} --mg-levels {:d} --qval {:f} --max-jd-size {:d} "
        "--max-gmres-size {:d} --n-mesh-refine {:d} --max-iter {:d} "
        "--qn-subspace {:d}".format(
            args.r0_frac,
            args.mg_levels,
            args.qval,
            args.max_jd_size,
        ...
        for vol in args.vol_frac:
            for omz in args.optimizer:
                ...
                if dom == "lbracket":
                    for rat in args.ratio:
                        ...
                        if os.path.isfile(os.path.join(prefix, "output_refine0.pkl")):
                            ...
                            options = (
                                "--domain {:s} --AR 1.0 --ratio {:f} --vol-frac {:f} "
                            ...
                            cases.append(
                                " ".join(
                                    [
                                        'echo "[{:4d}/{:4d}] running case: {:s}" && python'.format(
                                            case_num, n_cases, prefix
                                        ),
                                        args.exec,
                                        options,
                                        commons,
                ...
                # Otherwise, this is a cantilever-type domain
                    ...
                    for ar in args.AR:
                        prefix = "{:s}-a{:.1f}-v{:.1f}-{:s}".format(
                            domname(dom), ar, vol, omz
                        )
                        if not os.path.isdir(prefix):
                            os.mkdir(prefix)
                        if os.path.isfile(os.path.join(prefix, "output_refine0.pkl")):
                            ...
                            options = (
                                "--domain {:s} --AR {:f} --ratio 1.0 --vol-frac {:f} "
    ...
    if os.path.isfile(sh_name):
        print(
            "[generate_scripts.py] Warning: File {:s} exists! Replacing it...".format(
                sh_name
            )
        )
    ...
    for pi in range(args.partition):
        ...
        with open(pbs_name, "w") as f:
            ...
            f.write(
                "parallel -j 24 :::: cases-n{:d}-part{:d}.sh\n".format(case_num, pi + 1)
            )
