...
class LobattoQuad2 : public PlaneStressQuad<2> {
 public:
  LobattoQuad2(PlaneStressStiffness *_stiff) : PlaneStressQuad<2>(_stiff) {
    this->numGauss = 3;
...
class LobattoQuad3 : public PlaneStressQuad<3> {
 public:
  LobattoQuad3(PlaneStressStiffness *_stiff) : PlaneStressQuad<3>(_stiff) {
    this->numGauss = 4;
...
class LobattoQuad4 : public PlaneStressQuad<4> {
 public:
  LobattoQuad4(PlaneStressStiffness *_stiff) : PlaneStressQuad<4>(_stiff) {
    this->numGauss = 5;
...
class LobattoQuad5 : public PlaneStressQuad<5> {
 public:
  LobattoQuad5(PlaneStressStiffness *_stiff) : PlaneStressQuad<5>(_stiff) {
    this->numGauss = 6;
...
class PSTopo : public PlaneStressStiffness {
 public:
  ...
  PSTopo(TacsScalar _rho, TacsScalar E, TacsScalar nu, TacsScalar ys, double q,
         double eps, int _nodes[], double _weights[], int _nweights);
  ~PSTopo();

  void setDesignVars(const TacsScalar dvs[], int numDVs);
  void getDesignVars(TacsScalar dvs[], int numDVs);
  void getDesignVarRange(TacsScalar lb[], TacsScalar ub[], int numDVs);
  void calculateStress(const double pt[], const TacsScalar e[], TacsScalar s[]);
  ...
  TacsScalar getDVOutputValue(int dvIndex, const double pt[]) { return xw; }
...
class PSTopo4 : public PlaneStressStiffness {
 public:
  ...
  PSTopo4(TacsScalar _rho, TacsScalar E, TacsScalar nu, TacsScalar ys, double q,
          double eps, int *_nodes[], double *_weights[], int _nweights[]);
  ~PSTopo4();
