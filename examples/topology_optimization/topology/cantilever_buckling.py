...
def createTopoProblem(props, forest, order=2, nlevels=2,
    ...
    for i in range(nlevels-1):
        ...
        # Make the creator class
        creator = CreateMe(bcs, filters[-1], props)
        assemblers.append(creator.createTACS(forest,scale=Xscale,ordering=ordering))
        varmaps.append(creator.getMap())
        vecindices.append(creator.getIndices())
...
for ite in range(max_iterations):
    ...
    # Set an axial compressive load
    T = 2.0e7 # 1e8, 1e7
    aux = addFaceTraction(2, forest, 'load', assembler,
                          [-T, 0.0, 0.0])
    ...
    num_eigs = 10
    ks_weight = 50.0
    offset = -1.5e0#-2.5e4, 1e4
    scale = 1.0
    max_lanczos = 100
    tol = 1e-16
    problem.addBucklingConstraint(sigma, num_eigs, ks_weight,
                                  offset, scale,
                                  max_lanczos, tol)
