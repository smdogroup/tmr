...
class FrequencyConstraint:
    ...
    def constraint(self, fltr, mg):
        ...
        if self.contype == "semi-def":
            ...
            self.mg.assembleMatCombo(
                TACS.STIFFNESS_MATRIX, 1.0, TACS.MASS_MATRIX, -0.95 * self.omega**2
            )
            self.mg.factor()

            # Assemble the exact matrix at K + lambda*G
            self.assembler.assembleMatCombo(
                TACS.STIFFNESS_MATRIX, 1.0, TACS.MASS_MATRIX, -self.omega**2, self.mat
            )
        else:
        ...
        if self.contype == "semi-def":
            ...
            cval = self.eig_scale * (
                (eig_min - np.log(self.beta) / self.rho) - self.omega**2
            )
        ...
        for i in range(self.num_eigenvalues):
            ...
            if self.contype == "semi-def":
                self.assembler.addMatDVSensInnerProduct(
                    scale, TACS.STIFFNESS_MATRIX, self.W[i], self.W[i], dcdx
                )
                self.assembler.addMatDVSensInnerProduct(
                    -scale * self.omega**2,
                    TACS.MASS_MATRIX,
                ...
                self.assembler.addMatDVSensInnerProduct(
                    -scale * self.eigs[i], TACS.MASS_MATRIX, self.W[i], self.W[i], dcdx
                )
    ...
    def update_model(self, x, approx):
        ...
        self.M = (
            self.conscale
            * self.rho
            * (np.outer(self.eta, self.eta) - np.diag(self.eta))
        )
        ...
        for i in range(nmv):
            ...
            scale = self.conscale
            self.assembler.addMatDVSensInnerProduct(
                scale, TACS.STIFFNESS_MATRIX, self.W[i], self.W[i], dfdx
            )
            self.assembler.addMatDVSensInnerProduct(
                -scale * self.omega**2, TACS.MASS_MATRIX, self.W[i], self.W[i], dfdx
            )
        ...
        for k in range(npv):
            ...
            scale = self.conscale
            self.assembler.addMatDVSensInnerProduct(
                scale, TACS.STIFFNESS_MATRIX, self.W[i], self.W[j], dfdx
            )
            self.assembler.addMatDVSensInnerProduct(
                -scale * self.omega**2, TACS.MASS_MATRIX, self.W[i], self.W[j], dfdx
            )
...
class BucklingConstraint:
    ...
    def constraint(self, fltr, mg):
        ...
        if self.fltr is None:
            ...
            self.gmres = TACS.KSM(
                self.mg.getMat(), self.mg, 25, nrestart=20, isFlexible=1
            )
        ...
        for i in range(self.num_eigenvalues):
            ...
            if nconv < self.num_eigenvalues and self.comm.rank == 0:
                print(
                    "Extracted eigenvalue[%2d]: %25.15e error: %25.15e"
                    % (i, self.eigs[i], error)
                )
    ...
    def constraint_gradient(self, fltr, mg, vecs):
        ...
        for i in range(self.num_eigenvalues):
            ...
            self.assembler.evalMatSVSensInnerProduct(
                TACS.GEOMETRIC_STIFFNESS_MATRIX, self.W[i], self.W[i], self.adjoint
            )
    ...
    def update_model(self, x, approx):
        ...
        for i in range(nmv):
            ...
            # Compute the derivative of d(W[i]^{T}*G*W[i])/d(path)
            self.assembler.evalMatSVSensInnerProduct(
                TACS.GEOMETRIC_STIFFNESS_MATRIX, self.W[i], self.W[i], self.dfdu
            )
        ...
        for k in range(npv):
            ...
            # Compute the derivative of d(W[i]^{T}*G*W[i])/d(path)
            self.assembler.evalMatSVSensInnerProduct(
                TACS.GEOMETRIC_STIFFNESS_MATRIX, self.W[i], self.W[j], self.dfdu
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--omega", type=float, default=10.0, help="natural frequency constraint value"
    )
    p.add_argument("--load_factor", type=float, default=1.0, help="load factor value")
    p.add_argument(
        "--linearized",
        default=False,
        action="store_true",
    ...
    for step in range(max_iterations):
        ...
        if args.contype == "semi-def" and not args.linearized:
            ...
            eig_qn = ParOptEig.EigenQuasiNewton(
                qn, approx, index=eigenvalue_constraint_index
            )
        ...
        write_dvs_to_file(
            xopt, assembler, os.path.join(args.prefix, "dv_output%d.f5" % (step))
        )
