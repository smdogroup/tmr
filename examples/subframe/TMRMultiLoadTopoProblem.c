...
TMRMultiLoadTopoProblem::TMRMultiLoadTopoProblem( int _nlevels, 
						  TACSAssembler *_tacs[],
...
*/
TMRMultiLoadTopoProblem::~TMRMultiLoadTopoProblem(){
  for ( int k = 0; k < nlevels; k++ ){
    tacs[k]->decref();
...
*/
void TMRMultiLoadTopoProblem::setInitDesignVars( ParOptVec *vars ){
  if (vars){
...
/*
  Set the iteration count
*/
void TMRMultiLoadTopoProblem::setIterationCounter( int iter ){
  iter_count = iter;
}
...
*/
ParOptScalar TMRMultiLoadTopoProblem::getObjectiveScaling(){
  return obj_scale;
}
...
*/
void TMRMultiLoadTopoProblem::setObjectiveScaling( ParOptScalar scale ){
  obj_scale = scale;
}
...
*/
ParOptScalar TMRMultiLoadTopoProblem::getMassScaling(){
  return mass_scale;
}
...
*/
void TMRMultiLoadTopoProblem::setMassScaling( ParOptScalar scale ){
  mass_scale = scale;
}
...
*/
ParOptVec *TMRMultiLoadTopoProblem::createDesignVec(){
...
*/
int TMRMultiLoadTopoProblem::isSparseInequality(){ 
  return 0; 
}
...
*/
int TMRMultiLoadTopoProblem::isDenseInequality(){ 
  return 1;
}
...
*/
int TMRMultiLoadTopoProblem::useLowerBounds(){ 
  return 1; 
}
...
*/
int TMRMultiLoadTopoProblem::useUpperBounds(){
  return 1; 
}
...
*/ 
void TMRMultiLoadTopoProblem::getVarsAndBounds( ParOptVec *x, 
                                       ParOptVec *lb, 
                                       ParOptVec *ub ){
  if (use_inverse_vars){
    if (x){ x->set(1.05); }
...
*/
void TMRMultiLoadTopoProblem::setLinearization( double q, ParOptVec *xvec ){
...
*/  
int TMRMultiLoadTopoProblem::getLocalValuesFromBVec( TACSBVec *vec,
						     TacsScalar *xloc ){
...
/*
  ...
  size
*/
void TMRMultiLoadTopoProblem::setBVecFromLocalValues( const TacsScalar *xloc,
						      TACSBVec *vec ){
  TacsScalar *x_vals, *x_ext_vals;
  int size = vec->getArray(&x_vals);
  int ext_size = vec->getExtArray(&x_ext_vals);
...
*/
int TMRMultiLoadTopoProblem::evalObjCon( ParOptVec *pxvec, 
					 ParOptScalar *fobj, 
					 ParOptScalar *cons ){
  ...
  if (wrap){
    ...
    if (obj_scale < 0.0){
      obj_scale = 1.0/average_compliance;
    }

    // Set the compliance objective and the mass constraint
    *fobj = obj_scale*average_compliance;
...
*/
int TMRMultiLoadTopoProblem::evalObjConGradient( ParOptVec *xvec, 
						 ParOptVec *g, 
						 ParOptVec **Ac ){
...
*/
int TMRMultiLoadTopoProblem::evalHvecProduct( ParOptVec *xvec,
					      ParOptScalar *z, 
					      ParOptVec *zw,
					      ParOptVec *pxvec, 
					      ParOptVec *hvec ){
...
// ------------------------
void TMRMultiLoadTopoProblem::evalSparseCon( ParOptVec *x, 
					     ParOptVec *out ){}

// Compute the Jacobian-vector product out = J(x)*px
// --------------------------------------------------
void TMRMultiLoadTopoProblem::addSparseJacobian( double alpha, 
						 ParOptVec *x,
						 ParOptVec *px, 
						 ParOptVec *out ){}
...
// -----------------------------------------------------------------
void TMRMultiLoadTopoProblem::addSparseJacobianTranspose( double alpha, 
							  ParOptVec *x,
							  ParOptVec *pzw, 
							  ParOptVec *out ){}
...
// ------------------------------------------------------------
void TMRMultiLoadTopoProblem::addSparseInnerProduct( double alpha, 
						     ParOptVec *x,
						     ParOptVec *cvec, 
						     double *A ){}

// Write the output file
void TMRMultiLoadTopoProblem::writeOutput( int iter, ParOptVec *xvec ){
